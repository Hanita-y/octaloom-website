                ...
                <>
                  ...
                  <div style={{ ...dropBase, top: "calc(100% + 10px)", left: 0 }}>
                    <div style={{ position: "relative" }}
                      ...
                      <a href="https://www.octaloom.com/linkedin-growth-engine" style={dropItem}
      ...
      {isMobile && menuOpen && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "20px 32px 40px" }}>
          ...
          <button onClick={() => setLinkedinExpanded(prev => !prev)}
            style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 20, color: DEEP_PURPLE, padding: "11px 0", fontWeight: 600, borderBottom: "1px solid rgba(113,46,172,0.08)", fontFamily: FONT, background: "none", border: "none", cursor: "pointer", width: "100%", textAlign: "left" as const }}>
              ...
              <div style={{ display: "flex", alignItems: "center", borderTop: "1px solid #e0e0e0", padding: "3px 6px" }}>
                ...
                <div style={{ flex: 1, display: "flex", justifyContent: "center", position: "relative" }}>
                      ...
                      <motion.div key="picker"
                        ...
                        {Object.entries(REACTION_IMG).map(([type, src]) => (
                          <button key={type}
                            ...
                            style={{ background: "none", border: "none", cursor: "pointer", padding: "4px 5px", borderRadius: "50%", lineHeight: 1 }}>
                            <img src={src} alt={type} width={32} height={32} style={{ display: "block" }}/>
                          </button>
                        ))}
                      </motion.div>
                    )}
                  ...
                  <button
                    ...
                    {reactions[i]
                      ? <img src={REACTION_IMG[reactions[i]]} alt={reactions[i]} width={18} height={18} style={{ display: "block" }}/>
  ...
  useEffect(() => {
    ...
    const onClick = (e: MouseEvent) => {
      ...
      for (let i = 0; i < 6 + Math.floor(Math.random() * 4); i++) {
        ...
        particles.current.push({ x: e.clientX, y: e.clientY, vx: Math.cos(angle)*spd*(Math.random()>.5?1:-1), vy: -Math.sin(angle)*spd-1, sz: 18+Math.random()*12, op: 1, rot: Math.random()*360, rotSpd: (Math.random()-.5)*9, img: imgs.current[Math.floor(Math.random()*imgs.current.length)], life: 0, maxLife: 42+Math.random()*28 })
      }
    }
    ...
    const animate = () => {
      ...
      particles.current = particles.current.filter(p => {
        ...
        try { ctx.drawImage(p.img,-p.sz/2,-p.sz/2,p.sz,p.sz) } catch(e) {} ctx.restore(); return true
      })
